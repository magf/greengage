...
to generate a SQL dump archive. If the pull fails, the dump creation and upload
steps are skipped for that matrix entry.
...
across all matrix configurations by querying the GitHub Actions jobs API. If no
dumps were uploaded, the workflow fails with an error.
- **Controlled Execution:** Since the main CI workflow runs on `6.x` and `7.x`
branches only for push events (which occur after final merge of a PR), SQL dumps
...
version 7) and constructs the expected Docker image name using the commit SHA.
...
how many matrix jobs completed the `Upload SQL Dump` step successfully. Fails
the workflow if none succeeded; passes if at least one dump was created.
